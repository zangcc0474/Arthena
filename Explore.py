...
def draw_pie(dataset,variable):
    fig = {
    'data': [{'labels': [str(item) for item in dataset[variable].value_counts().index.tolist()],
              'values': dataset[variable].value_counts().tolist(),
              'type': 'pie'}],
    'layout': {'title': 'Product Distribution on ' + str(variable)}
     }
    return fig